...
import { ensureFileMatchesGradeName, ensureFileRenamedOnDisk, sanitizeGradeFilename } from './sanitize';
    ...
    for (const candidate of allSongsPool) {
      ...
      if (!usedInBlock.has(key) && !usedArtistsInBlock.has(normalizedArtist) && !ctx.isRecentlyUsed(candidate.title, candidate.artist, timeStr, isFullDay)) {
        ...
        if (libraryResult.exists) {
          ...
          const sanitizedFilename = await finalizeGradeFilename(correctFilename, candidate.artist, candidate.title);
          return `"${sanitizedFilename}"`;
        }
      }
    }
