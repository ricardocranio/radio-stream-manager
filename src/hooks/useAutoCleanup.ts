 ...
 * - Scraped songs older than 24h (Supabase - optional)
 * 
...
export function useAutoCleanup() {
  ...
  const performCleanup = useCallback(async () => {
    ...
    try {
      ...
      if (!error) {
        console.log('[CLEANUP] 🧹 Cleaned old Supabase data');
      }
    } catch (err) {
    ...
    if (cleanedCount > 0) {
      console.log(`[CLEANUP] 🧹 Cleaned ${cleanedCount} old entries (>${MAX_DATA_AGE_HOURS}h)`);
    }
  }, []);
